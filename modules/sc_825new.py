...
def sentence_checker(context: list, responser:dict, scenarios: dict, vocab: dict, grammar: dict, world: dict, skill: dict, progress: dict, archive_lock=nullcontext()):
    ...
    for raw_key, level in proficiency.items():
        ...
        #   intermediate(0.6): 仅一次简单提及（无明显搭配/结构证据）
        ...
        if table == 'vocab':
            ...
            # 记下证据用于后续自适应步长
            evidence = {'occ': occ, 'collocation': bool(collocation), 'llm_level': llm_level}
        elif table == 'grammar':
            override_level = level
            evidence = {'grammar_confidence': float(grammar_confidence), 'llm_level': level}

        filtered_items.append((table, uid, content, override_level, evidence))

    for table, uid, content, level, evidence in filtered_items:
        ...
        def _alpha_for_vocab(x, evidence):
            # 基础步长
            alpha0 = 0.35
            occ = int(evidence.get('occ', 1) or 1)
            coll = bool(evidence.get('collocation', False))
            llm_level = str(evidence.get('llm_level', 'intermediate')).lower()
            # 出现/搭配权重
            w_occ = 1.5 if (occ >= 2 or coll) else 1.0
            # LLM 等级权重
            w_llm_map = {'beginner': 0.3, 'intermediate': 0.7, 'advanced': 1.0, 'proficient': 1.2}
            w_llm = w_llm_map.get(llm_level, 0.7)
            a = alpha0 * w_occ * w_llm
            a = _clip(a, 0.1, 0.7)
            # 边际收益递减（靠近 0/1 放缓）
            diminish = 1.0 - abs(2.0 * x - 1.0)
            return a * diminish

        def _alpha_for_grammar(x, evidence):
            alpha0 = 0.35
            conf = float(evidence.get('grammar_confidence', 0.0))
            a = alpha0 * (0.5 + 0.5 * conf)
            a = _clip(a, 0.1, 0.6)
            diminish = 1.0 - abs(2.0 * x - 1.0)
            return a * diminish

        def _update_value(x, y, a, delta_cap=0.25):
            target = x + a * (y - x)
            # 单轮变动上限
            diff = target - x
            if abs(diff) > delta_cap:
                target = x + (delta_cap if diff > 0 else -delta_cap)
            return _clip(target, 0.0, 1.0)

        is_vocab = (table == 'vocab')
        x_updated = []
        if is_vocab:
            # 误用：issues 提到该词 或 最终等级为 beginner
            is_beginner = (str(level).lower() == 'beginner')
            misuse = _issues_mention_word(issues, content) or is_beginner
            # 参数：误用负向步长与目标
            y_err = 0.15
            alpha_minus = 0.8
            ...
            def _apply(x):
                if misuse:
                    return _update_value(x, y_err, alpha_minus, delta_cap=0.25)
                else:
                    a = _alpha_for_vocab(x, evidence)
                    return _update_value(x, score, a, delta_cap=0.25)
            x_updated = rows.apply(_apply)
        else:
            def _apply(x):
                a = _alpha_for_grammar(x, evidence)
                return _update_value(x, score, a, delta_cap=0.25)
            x_updated = rows.apply(_apply)
