...
class AudioGenerator:
    ...
    """def generate_and_play_audio(self, text: str, save_filename: Optional[str] = None, play_audio: bool = False,
        ...
        ===== 修改说明 =====
        - 不再在服务器端播放音频，只生成并保存。
        - 如果需要流式播放给前端，用 tts_stream_generator。
        '''
        if self.piper_voice is None:
            if not self.init_tts_pipeline(language):
                return None

        audio_chunks = self.generate_audio_chunks(text, language=language)
        saved_path = None
    ...
    """def tts_stream_generator(self, text: str, save_filename: Optional[str] = None,
                     first_chunk_time_callback: Optional[Callable[[], None]] = None,
                     language: str = 'english',
                     callback: Optional[Callable[[str], None]] = None):
        '''
        ===== 修改说明 =====
        - 边 yield 给前端 (gr.Audio streaming)，边写入完整 wav 文件。
        '''
        if self.piper_voice is None:
            if not self.init_tts_pipeline(language):
                return
        ...
        wf = None
        if save_filename:
            saved_path = os.path.join(self.audio_dir, f"{save_filename}.wav")
            import soundfile as sf
            wf = sf.SoundFile(saved_path, mode="w", samplerate=self.sample_rate, channels=1, subtype="PCM_16")

        try:
            for chunk in audio_chunks:
                arr = chunk.reshape(1, -1)  # (1, n)
                # ---- 写文件 ----
                if wf is not None:
                    wf.write(chunk)

                if first_yield and callable(first_chunk_time_callback):
                    try:
                        first_chunk_time_callback()
                    except Exception as e:
                        print(f"first_chunk_time_callback error: {e}")
        ...
        # ==== 新增: 完成后的回调 ====
        if saved_path and callable(callback):
            try:
                callback(saved_path)
            except Exception as e:
                print(f"callback error: {e}")
